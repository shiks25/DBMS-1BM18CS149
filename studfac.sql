...
/*find the names of all Juniors(jr) who are enrolled in a class taught by 'Monica'; ans='Nicki'*/
select s.sname from student s,enrolled e,class c,faculty f where s.snum=e.snum and e.cname=c.cname and c.fid =f.fid and s.lvl='jr'and f.fname='Monica';

/*find the names of all classes that either meet in room R128 or have five or more students enrolled ; ans = class 10 and class 5*/
...
/*find the names of all students who are enrolled in two classes that meet at the same time; ans = Selena*/
select distinct sname from student where snum in(select e1.snum from enrolled e1,enrolled e2,class c1,class c2 where e1.snum=e2.snum and e1.cname <> e2.cname 
                                             and e1.cname=c1.cname and e2.cname=c2.cname and c1.meetsat=c2.meetsat );
                                             
 /*find names of  faculty members who teach in every room in which some class is taught; ans=Shamala*/
 ...
 /*find names of  faculty members for whom the combined enrollment of the courses that they teach is less than five;ans= Sarita,Monica,Raji,Manjunath*/
...
WHERE C.cname = E.cname AND C.fid = F.fid) <5;
 ...
 /*find the names of the students who are not enrolled in any class;ans='Rahul'*/
select distinct sname from student where snum not in(select snum from enrolled );

/*for each age that appears in students find the level value that apperas most often; ans=19-sr,20-jr,21-fr*/
